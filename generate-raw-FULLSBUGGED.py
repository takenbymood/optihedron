...
rots = []
rots.append([[0.0, 1.0, 0.0],0.0])
rots.append([[0.0, 1.0, 0.0],3.141])
rots.append([[-1.0,0.0, 0.0],1.571])
rots.append([[1.0, 0.0, 0.0],1.571])
rots.append([[0.0,-1.0, 0.0],1.571])
rots.append([[0.0, 1.0, 0.0],1.571])

#fix1: remove liney36
designmode = [liney36, patchy36, patchy42, patchy33, patchy32, patchy34, patchy30, liney36, liney42, liney33, liney32, liney34, liney30]
...
def routine(params):
    ...
    ind = indmode
    OUTDIR = 'models/raw/out'
    RUNDIR = 'models/raw/run'
    ...
    dumpdata = [contactData, maxcontactligs, np, fixedrot[0], fixedrot[1], f, b]
    pickleName = '{}-{}-{}'.format(indname, eps, rotidx)            
    pickle.dump(dumpdata, open('models/pickles/{}.p'.format(pickleName), 'wb'))
    if os.path.exists(outFilePathProcessed):
        os.remove(outFilePathProcessed)
    if os.path.exists(outFilePath):
        os.remove(outFilePath)
    sim.deleteFiles()                    
#fix 3: remove files post

#fix 4: remove short params list
params = []
epstotal = [250,240,230,220,210,200]
for indmode, indname in zip(designmode, designname):
    for rotidx, fixedrot in enumerate(rots):
        for eps in epstotal:
            if len(params) < 2:
                params.append((indmode,indname,rotidx,fixedrot,eps))

WORKERS = 2
mapping = pools.ProcessPool(WORKERS).map
mapping(routine, params)