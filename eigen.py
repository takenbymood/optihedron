...
with open(spectrumFilePath, 'w') as specFile:
    ...
    specWriter.writerows([['id','e','n','fitness','budtime','budfrac','spectrum']])
...
for i in dbSession.query(Particle).yield_per(100):
	...
	for n in i.network.nodes(data=True):
		if n[1]['weight'] != 0:
			nodelist.append(n[0])
	spectrum = np.linalg.eigvals(nx.to_numpy_matrix(i.network,nodelist=nodelist))
	spectra.append([str(int(i.pID)),str(float(i.avgEps)),str(float(i.nligands)),str(float(i.fitness)),str(float(i.budTime)),str(float(i.budPerc)),str(list(spectrum))])
	if len(spectra) == 100:
